...
for var multiple : Int = 0;multiple <= 3; ++multiple{
           var seven : Int = multiple * 7
    let six : Int = seven - 1
        print ("\(six)")
}
...
print ("Hello \(name) the number of characters in your name is \(name.characters.count)")
...
for var loop : Int = 1;loop <= 10; ++loop {
    let random = Int(arc4random_uniform(101))
    if random % 3 == 0  && random % 5 == 0{
        print("\(random) is a BUZZ and FIZZ")
    }
    else if random % 5 == 0 {
        print("\(random) is a FIZZ")
    }
    else if random % 3 == 0 {
        print("\(random) is a BUZZ")
    }
    else {print ("\(random) is neither a buzz or a fizz")
...
for var fiba : Int = 1;fiba <= 100; ++fiba {
    if fiba % 2 == 0 {
    var x = ++fiba
    }
        else {var x = fiba
